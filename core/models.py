...
class LGM(nn.Module):
    def __init__(
        ...
        self.rgb_act = (
            lambda x: 0.5 * torch.tanh(x) + 0.5
        )  # NOTE: may use sigmoid if train again
    ...
    def forward(self, data, step_ratio=1, run=None):
        ...
        gt_images = data[
            "images_output"
        ]  # [B, V, 3, output_size, output_size], ground-truth novel views
        gt_masks = data[
            "masks_output"
        ]  # [B, V, 1, output_size, output_size], ground-truth masks
        ...
        if run is not None:
            if self.training:
                run["train/loss_mse"].log(loss_mse.item())
            else:
                run["eval/loss_mse"].log(loss_mse.item())
        loss = loss + loss_mse
        ...
        if self.opt.lambda_lpips > 0:
            loss_lpips = self.lpips_loss(
                ...
                F.interpolate(
                    gt_images.view(-1, 3, self.opt.output_size, self.opt.output_size)
                    * 2
                    - 1,
                    (256, 256),
                    mode="bilinear",
                ...
                F.interpolate(
                    pred_images.view(-1, 3, self.opt.output_size, self.opt.output_size)
                    * 2
                    - 1,
                    (256, 256),
                    mode="bilinear",
